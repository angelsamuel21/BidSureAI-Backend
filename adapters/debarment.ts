...
export class DebarmentAdapter implements GovernmentVerificationAdapter {
  portalName = 'DEBARMENT' as const

  async verify(bidderNameOrPan: string): Promise<PortalVerificationRecord> {
    ...
    const debarredEntities: Record<string, { debarmentAgency: string; debarredFrom: string; debarredUntil: string; reason: string }> = {
      'PQR TECHNOLOGIES': {
    ...
    // Check matches
    let hit: { debarmentAgency: string; debarredFrom: string; debarredUntil: string; reason: string } | null = null
