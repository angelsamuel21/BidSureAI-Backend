...
export class MakeInIndiaAdapter implements GovernmentVerificationAdapter {
  portalName = 'MII' as const

  async verify(
    claimedPercentage: string,
    context?: { minRequiredPercent?: number; oemDeclaredPercent?: number; selfDeclarationDoc?: string }
  ): Promise<PortalVerificationRecord> {
