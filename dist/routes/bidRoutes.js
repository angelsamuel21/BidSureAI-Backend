...
router.post('/', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        const created = await Bid_1.Bid.create({
...
router.get('/', async (req, res) => {
    try {
        ...
        const bidDocs = await Bid_1.Bid.find(query).lean();
        if (bidDocs && bidDocs.length > 0) {
            bids = bidDocs.map((b) => ({
                ...
                officerDecision: b.officerDecision
                    ? {
                        ...b.officerDecision,
                        timestamp: b.officerDecision.timestamp instanceof Date
                ...
                evaluation: b.evaluation
                    ? {
                        ...b.evaluation,
                        evaluatedAt: b.evaluation.evaluatedAt instanceof Date
                ...
                clarifications: (b.clarifications || []).map((c) => ({
        ...
        if (status && status !== 'all') {
            bids = bids.filter((b) => {
                if (status === 'Compliant')
                    return b.evaluation?.recommendation === 'COMPLIANT';
                if (status === 'Review Required')
                    return b.evaluation?.recommendation === 'REQUIRES_MANUAL_REVIEW';
                if (status === 'Non-Compliant')
                    return b.evaluation?.recommendation === 'NON-COMPLIANT';
                return b.status === status;
            });
        }
        if (search) {
            bids = bids.filter((b) => b.bidderName.toLowerCase().includes(search) ||
...
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        await (0, mongodb_1.connectToDatabase)();
        const b = await Bid_1.Bid.findById(id).lean();
        if (!b) {
        ...
        const docRecords = await Document_1.DocumentModel.find({ bidId: id }).lean();
...
router.patch('/:id', async (req, res) => {
    try {
        ...
        await (0, mongodb_1.connectToDatabase)();
        const bid = await Bid_1.Bid.findOne({ _id: id });
        if (!bid) {
            return res.status(404).json({ success: false, message: 'Bid not found', error: 'BID_NOT_FOUND' });
        }
