...
async function main() {
    ...
    const defaultHash = crypto_1.default.createHash('sha256').update('password123').digest('hex');
    ...
    const bid1Verifs = [
        {
            ...
            evidence: 'GSTIN 09ABCDE1234F1Z5 is Active, Regular taxpayer in Demo Sandbox.',
            explanation: 'GST registration active and verified with GSTN Sandbox.',
            score: 100,
        },
        {
            ruleId: 'rule-pan',
            ...
            explanation: 'PAN verified in Demo Sandbox.',
            score: 100,
        },
        {
            ruleId: 'rule-udyam',
            ...
            explanation: 'Udyam verified in Demo Sandbox.',
            score: 100,
        },
        {
            ruleId: 'rule-mii',
    ...
    const bid1OfficerDecision = {
        decision: 'APPROVE',
        justification: 'All statutory, financial, and technical credentials satisfy tender requirements. Active GST and PAN verified.',
    ...
    for (const d of bid1Docs)
        await docsCol.insertOne({ ...d, bidId: bid1Id, createdAt: new Date() });
    for (const v of bid1Verifs)
        await verifsCol.insertOne({ ...v, bidId: bid1Id, createdAt: new Date() });
    await compCol.insertOne({
        bidId: bid1Id,
        score: 94,
        riskLevel: 'LOW',
        status: 'COMPLIANT',
        summary: 'Bidder satisfies all statutory registration criteria and meets Class-I Local Supplier qualifications. Zero debarment found.',
        evaluatedAt: new Date('2026-08-18T10:42:00Z'),
    });
    ...
    const bid2Verifs = [
        {
            ...
            explanation: 'GST registration active and verified in Demo Sandbox.',
            score: 100,
        },
        {
            ruleId: 'rule-pan',
        ...
        },
        {
            ...
            evidence: 'Self-declaration claims 65%, but OEM breakdown document specifies only 42% (below 50% minimum).',
    ...
    const bid2Eval = {
        ...
        aiSummary: 'Material discrepancy detected: Bidder claims 65% local content in self-declaration, but OEM document specifies only 42%. Officer clarification required.',
    ...
    const bid2OfficerDecision = {
        ...
        justification: 'Discrepancy detected: The submitted local-content document indicates 42%, while the tender requires at least 50%.',
        officerName: 'Riya Kapoor',
        officerRole: 'PROCUREMENT_OFFICER',
        ...
        clarifications: [
            {
                ...
                message: 'Discrepancy detected: The submitted local-content document indicates 42%, while the tender requires at least 50%. Please provide certified CA verification.',
                queryText: 'Discrepancy detected: The submitted local-content document indicates 42%, while the tender requires at least 50%. Please provide certified CA verification.',
    ...
    for (const d of bid2Docs)
        await docsCol.insertOne({ ...d, bidId: bid2Id, createdAt: new Date() });
    for (const v of bid2Verifs)
        await verifsCol.insertOne({ ...v, bidId: bid2Id, createdAt: new Date() });
    await compCol.insertOne({
        bidId: bid2Id,
        score: 68,
        riskLevel: 'HIGH',
        status: 'REQUIRES_MANUAL_REVIEW',
        summary: 'The submitted local-content document indicates 42%, while the tender requires at least 50%. Clarification recommended.',
        evaluatedAt: new Date('2026-08-16T15:30:00Z'),
    });
    ...
    const bid3Verifs = [
        ...
        },
        {
            ...
            explanation: 'GST registration active in Demo Sandbox.',
            score: 100,
        },
        {
            ruleId: 'rule-pan',
    ...
    const bid3Eval = {
        ...
        aiSummary: 'Critical disqualification flags detected. Bidder is debarred on official registries.',
    ...
    for (const d of bid3Docs)
        await docsCol.insertOne({ ...d, bidId: bid3Id, createdAt: new Date() });
    for (const v of bid3Verifs)
        await verifsCol.insertOne({ ...v, bidId: bid3Id, createdAt: new Date() });
    await compCol.insertOne({
        bidId: bid3Id,
        score: 38,
        riskLevel: 'CRITICAL',
        status: 'NON-COMPLIANT',
        summary: 'Active debarment detected on Ministry of Petroleum & Natural Gas / CVC blacklist. Mandatory rejection required.',
        evaluatedAt: new Date('2026-08-12T11:00:00Z'),
    });
    ...
    const h1 = calculateHash(GENESIS_HASH, 'TENDER_PUBLISHED', t1, 'Riya Kapoor', `Tender GEM/2026/001 published with 6 mandatory/technical compliance rules.`);
    ...
    const h2 = calculateHash(h1, 'BID_DOCUMENTS_INGESTED', t2, 'Riya Kapoor', `Bid submitted by ABC Industries Pvt. Ltd. with 5 PDF verification documents (SHA-256 verified).`);
    await auditLogsCol.insertOne({
        userId: user1.insertedId.toString(),
        actor: 'Riya Kapoor',
        role: 'PROCUREMENT_OFFICER',
    ...
    const h3 = calculateHash(h2, 'COMPLIANCE_EVALUATION_COMPLETED', t3, 'Automated Compliance Pipeline', `Automated compliance score generated: 94/100 (Risk: LOW). Recommendation: COMPLIANT.`);
    await auditLogsCol.insertOne({
    ...
    const h4 = calculateHash(h3, 'OFFICER_DECISION_APPROVE', t4, 'Riya Kapoor', `Procurement Officer approved bid for ABC Industries Pvt. Ltd.. Justification: All credentials satisfied.`);
    await auditLogsCol.insertOne({
        userId: user1.insertedId.toString(),
        actor: 'Riya Kapoor',
        role: 'PROCUREMENT_OFFICER',
