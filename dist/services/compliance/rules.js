...
exports.standardRules = [
        ...
        evaluate: (ctx) => {
            ...
            if (isFailed) {
                return {
                    ...
                    explanation: 'CRITICAL FAILURE: Bidder is actively debarred/blacklisted by a government authority. In accordance with GeM Incident Management Policy and GFR 2017 Rule 151, the bid must be rejected.',
                    confidence: 0.99,
    ...
    // 2. GST Registration & Legal Name Alignment
    {
        ...
        evaluate: (ctx) => {
            ...
            const doc = ctx.documents.find((d) => d.documentType === 'GST_REGISTRATION_CERTIFICATE' || (d.name || '').toLowerCase().includes('gst'));
    ...
    // 3. PAN Verification & Cross-Consistency with GSTIN
    {
        ...
        evaluate: (ctx) => {
            ...
            const doc = ctx.documents.find((d) => d.documentType === 'PAN_CARD' || (d.name || '').toLowerCase().includes('pan'));
            if (!record || record.status === 'FAILED') {
                return {
                    ruleId: 'rule-pan',
    ...
    // 4. Udyam / MSME Registration & PPP Order 2012 Exemption Status
    {
        ...
        evaluate: (ctx) => {
            ...
            const doc = ctx.documents.find((d) => d.documentType === 'UDYAM_MSME_CERTIFICATE' || (d.name || '').toLowerCase().includes('udyam'));
    ...
    // 5. Make in India (MII) / Local Content Compliance
    {
        ...
        mandatory: true,
        defaultWeight: 15,
        evaluate: (ctx, customReq) => {
            const minReq = customReq?.criteriaDetails?.minLocalContentPercent ?? 50;
            const record = ctx.portalRecords.mii;
            const doc = ctx.documents.find((d) => d.documentType === 'MAKE_IN_INDIA_DECLARATION' || (d.name || '').toLowerCase().includes('local content'));
    ...
    // 6. OEM Authorization Letter
    {
        ...
        defaultWeight: 15,
        evaluate: (ctx) => {
            const doc = ctx.documents.find((d) => d.documentType === 'OEM_AUTHORIZATION_LETTER' || (d.name || '').toLowerCase().includes('oem'));
            if (!doc) {
                return {
                    ruleId: 'rule-oem',
                    ...
                    explanation: 'In accordance with GeM General Terms and Conditions (GTC), non-OEM bidders must submit a valid Manufacturer Authorization Form. No OEM authorization document was found in this submission.',
                    confidence: 0.96,
                    source: 'DOCUMENT_EXTRACTED',
                };
            }
            ...
            if (isValid) {
                return {
                    ...
                    evidence: 'OEM Authorization Letter contains valid tender number reference, warranty commitment, and authorized signatory stamp.',
