...
class EntityExtractor {
    extractFromText(text, fileName = '') {
        ...
        const localContentMatch = clean.match(/(?:local\s+content|local\s+value\s+addition|indigenous\s+content)[\s:]*([0-9]{1,3}(?:\.[0-9]+)?)\s*%/i);
        if (localContentMatch) {
            entities.localContentPercent = parseFloat(localContentMatch[1]);
        }
        // 6. Annual Turnover
        const turnoverMatch = clean.match(/(?:annual\s+turnover|average\s+turnover|turnover)[\s:]*(?:INR|Rs\.?|₹)?\s*([0-9]+(?:\.[0-9]+)?)\s*(?:cr|crore|crores)?/i);
        if (turnoverMatch) {
        ...
        const nameMatch = clean.match(/(?:M\/s\.?|Name\s+of\s+Enterprise|Legal\s+Name|Company\s+Name)[\s:]+([A-Z0-9\s.,&-]{3,50})(?:\r?\n|$)/i);
        if (nameMatch) {
        ...
        else if (lower.includes('permanent account number') ||
        ...
        else if (lower.includes('local content') ||
...
exports.default = exports.entityExtractor;
