import { AIEvaluationRequest, AIEvaluationResponse } from './types'
import { AIFinding } from '@/types'

export class AIVerificationEngine {
  async analyze(request: AIEvaluationRequest): Promise<AIEvaluationResponse> {
    ...
    const hasOemIssue =
      oemRule?.status === 'FAIL' ||
      request.ruleDiscrepancies.some(
        (d) => d.toLowerCase().includes('oem') || d.toLowerCase().includes('manufacturer authorization')
      )
    ...
    if (hasOemIssue) {
      findings.push({
        ...
        description:
          'Mandatory OEM Authorization Letter not uploaded. In accordance with GeM General Terms and Conditions (GTC), non-OEM bidders must submit a valid Manufacturer Authorization Form.',
    ...
    if (hasMiiMissingIssue && !hasLocalContentIssue) {
      findings.push({
        ...
        description:
          'Mandatory Make in India (MII) local content declaration was not submitted with bid documents.',
        documentEvidence: 'Document not uploaded',
    ...
    if (hasLocalContentIssue) {
      ...
      const miiDoc = request.documents.find(
        (d) => d.type === 'MAKE_IN_INDIA_DECLARATION' || d.name?.toLowerCase().includes('local content')
      )
      findings.push({
        severity: 'HIGH',
        title: 'Make in India Local Content Percentage Discrepancy',
        description:
          claimedVal !== undefined
    ...
    if (hasGstIssue) {
      const gstDoc = request.documents.find(
        (d) => d.type === 'GST_REGISTRATION_CERTIFICATE' || d.name?.toLowerCase().includes('gst')
      )
      findings.push({
        severity: 'WARNING',
        title: 'Minor Trade Name Variation in GST Record',
        description:
          'The registered trade name in GST Portal displays minor suffix variation compared to the bid submission documents.',
    ...
    if (findings.length === 0 && !hasAnyFailure) {
      ...
      const oemDoc = request.documents.find(
        (d) => d.type === 'OEM_AUTHORIZATION_LETTER' || d.name?.toLowerCase().includes('oem')
      )
      if (oemDoc) {
        findings.push({
          severity: 'INFO',
...
export const aiVerificationEngine = new AIVerificationEngine()
export default aiVerificationEngine
