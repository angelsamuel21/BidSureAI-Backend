import { ComplianceEvaluationContext } from './types'
import { standardRules } from './rules'
import { RuleEvaluationResult, RiskLevel, Recommendation, TenderRequirement } from '@/types'
...
export class ComplianceEngine {
  evaluate(context: ComplianceEvaluationContext): {
    ...
    for (const rule of standardRules) {
      ...
      const customReq = context.tenderRequirements.find(
        (r) => r.code === rule.code || r.name.toLowerCase() === rule.name.toLowerCase()
      )
    ...
    let recommendation: Recommendation = 'COMPLIANT'
