import { TenderRequirement, UploadedDocument } from '@/types'

export const CANONICAL_28_FEATURE_KEYS = [
  'gst_required',
  'pan_required',
  'udyam_required',
  'itr_required',
...
export type CanonicalFeatureKey = typeof CANONICAL_28_FEATURE_KEYS[number]
export type FeatureVector = Record<CanonicalFeatureKey, number>
...
export interface FeatureGeneratorInput {
  ...
  portalRecords?: Record<string, any>
  ruleResults?: Array<{
    ruleId: string
    code?: string
    status: string
...
export class FeatureGeneratorService {
   ...
   */
  generateFeatures(input: FeatureGeneratorInput): FeatureVector {
    ...
    const isReq = (code: string, ...keywords: string[]): number => {
      if (reqCodes.has(code.toUpperCase())) return 1
      for (const kw of keywords) {
        if (reqNames.some((n) => n.includes(kw.toLowerCase()))) return 1
      }
      return 0
    }
    ...
    const hasDoc = (...types: string[]) => types.some((t) => docTypes.has(t.toUpperCase()))
    ...
    const gst_status_encoded =
      gstRecord?.status === 'VERIFIED' && gstRecord?.data?.status === 'Active'
        ? 1
        : hasDoc('GST_REGISTRATION_CERTIFICATE') && !gstRecord?.status?.includes('FAIL')
        ? 1
        : 0

    const pan_status_encoded =
      panRecord?.status === 'VERIFIED'
        ? 1
    ...
    const udyam_status_encoded =
      udyamRecord?.status === 'VERIFIED' || (!udyam_required && !bidder.udyamNumber)
        ? 1
        : hasDoc('UDYAM_MSME_CERTIFICATE')
        ? 1
        : 0

    const itr_status_encoded =
      hasDoc('ITR') || panRecord?.data?.itrFilingStatus === 'COMPLIANT' || !itr_required
        ? 1
        : 0

    const oemRule = rules.find((r) => r.ruleId === 'rule-oem')
    const oem_status_encoded =
      oemRule ? (oemRule.status === 'PASS' ? 1 : 0) : hasDoc('OEM_AUTHORIZATION_LETTER') || !oem_required ? 1 : 0

    const experience_status_encoded =
      hasDoc('EXPERIENCE_CERTIFICATE') || (isMsme && turnover_required) || !experience_required ? 1 : 0

    const turnover_status_encoded =
      hasDoc('TURNOVER_CERTIFICATE') || isMsme || !turnover_required ? 1 : 0

    const miiRule = rules.find((r) => r.ruleId === 'rule-mii')
    const make_in_india_status_encoded =
      miiRule ? (miiRule.status === 'PASS' ? 1 : 0) : miiRecord && !miiRecord.data?.hasDiscrepancy ? 1 : 0
    ...
    if (isDebarred) critical_failures += 1
    if (gst_required && gst_status_encoded === 0 && docs.length > 0) critical_failures += 1
    if (rules.some((r) => r.mandatory && r.status === 'FAIL')) critical_failures += 1
