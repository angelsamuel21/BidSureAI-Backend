...
import { sandboxGateway, mlServiceAdapter } from '@/adapters'
import { complianceEngine } from '@/services/compliance'
import { aiVerificationEngine } from '@/services/ai'
import { featureGenerator } from '@/services/featureGenerator'
import {
  Bid as BidType,
  Bidder as BidderType,
...
export class VerificationPipelineService {
   ...
   */
  async executePipeline(bidId: string): Promise<{
    bid: BidType
    ...
    if (typeof bidId === 'string' && /^[0-9a-fA-F]{24}$/.test(bidId)) {
      bidDoc = await Bid.findById(bidId)
    }
    if (!bidDoc) {
      bidDoc = await Bid.findOne({ $or: [{ id: bidId }, { _id: bidId }] }).catch(() => null)
    }
    if (!bidDoc) {
    ...
    const tenderDoc =
      (await Tender.findOne({ $or: tenderOrClauses })) || (await Tender.findOne())
    ...
    const requirements = (tenderDoc?.requirements as any[]) || []

    // 3. Fetch Uploaded Documents for this Bid
    const docRecords = await DocumentModel.find({ bidId }).lean()
    let uploadedDocs: UploadedDocument[] = []

    if (docRecords && docRecords.length > 0) {
      uploadedDocs = docRecords.map((d: any) => ({
        id: d._id.toString(),
        bidId,
      ...
      uploadedDocs = bidDoc.documents.map((d: any) => ({
    ...
    let extractedUdyam: string | undefined = undefined
    let extractedLegalName: string | undefined = undefined
    let claimedLocalContent: number | undefined = undefined
    let oemLocalContent: number | undefined = undefined

    for (const doc of uploadedDocs) {
      const fields = doc.extractedFields || {}
      if (fields.gstin && !extractedGstin) extractedGstin = String(fields.gstin).trim().toUpperCase()
      if (fields.pan && !extractedPan) extractedPan = String(fields.pan).trim().toUpperCase()
      if (fields.udyamNumber && !extractedUdyam) extractedUdyam = String(fields.udyamNumber).trim().toUpperCase()
      if (fields.legalName && !extractedLegalName) extractedLegalName = String(fields.legalName).trim()
      if (fields.companyName && !extractedLegalName) extractedLegalName = String(fields.companyName).trim()
    ...
    let bidderDoc: any = null
    if (typeof bidDoc.bidderId === 'string' && /^[0-9a-fA-F]{24}$/.test(bidDoc.bidderId)) {
      bidderDoc = await Bidder.findById(bidDoc.bidderId).catch(() => null)
    }
    if (!bidderDoc) {
      bidderDoc = await Bidder.findOne({
        $or: [
    ...
    if (bidderDoc) {
      if (extractedGstin) bidderDoc.gstin = extractedGstin
      if (extractedPan) bidderDoc.pan = extractedPan
      if (extractedUdyam) bidderDoc.udyamNumber = extractedUdyam
      if (isDebarred) bidderDoc.isDebarred = true
      await bidderDoc.save()
    } else {
      bidderDoc = await Bidder.create({
        name: bidderName,
    ...
    const bidderDomain: BidderType = {
      ...
      category: (bidderDoc.category as any) || 'MICRO',
      isDebarred: isDebarred || bidderDoc.isDebarred,
    }

    // 6. Run Statutory Verification Adapters
    const portalRecords = await sandboxGateway.verifyAll({
    ...
    const ruleEvaluation = complianceEngine.evaluate({
    ...
    const mlFeatures = featureGenerator.generateFeatures({
      tenderRequirements: requirements,
      bidder: bidderDomain,
      documents: uploadedDocs,
      portalRecords,
    ...
    const mlPrediction = await mlServiceAdapter.predict({
      task: 'all',
      tenderTitle,
    ...
    let finalRiskLevel: RiskLevel = ruleEvaluation.riskLevel
    let finalRecommendation: RecommendationType = ruleEvaluation.recommendation
    ...
    if (isDebarred) {
      ...
      if (mlPrediction.riskLevel === 'CRITICAL' || mlPrediction.riskLevel === 'HIGH') {
        finalRiskLevel = mlPrediction.riskLevel as RiskLevel
      }
    ...
    const aiAnalysis = await aiVerificationEngine.analyze({
    ...
    const initialStatus =
      finalRecommendation === 'COMPLIANT' ? 'EVALUATED' : 'CLARIFICATION_REQUESTED'
    ...
    bidDoc.status = initialStatus as any
    bidDoc.evaluation = evaluation as any
    ...
    await ComplianceResult.findOneAndUpdate(
      { bidId },
      {
        bidId,
        score: finalScore,
    ...
    await Verification.deleteMany({ bidId })
    ...
    const finalBid: BidType = {
      ...
      documents: uploadedDocs,
      evaluation,
      clarifications: (bidDoc.clarifications || []) as any,
      officerDecision: bidDoc.officerDecision as any,
    }
...
export default verificationPipelineService
